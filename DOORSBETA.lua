    ...
    PlrSection:NewButton("ОТКРЫТЬ ПОРТАЛ В ROOMS", "Вы бесплатно открываете портал в ROOMS.", function()
        ...
        workspace.CurrentRooms["60"].RoomsDoor_Entrance.Door.EnterPrompt.HoldDuration = 0
    end)
    PlrSection:NewButton("ВЗЯТЬ БАНКУ ДЖЕФФА", "Используйте после 52 Двери и после Раша.", function()
        workspace.CurrentRooms["52"].TipJarPickup.ModulePrompt.Enabled = true
    end)
    ...
    PlrSection:NewButton("ОТКРЫТЬ ПОРТАЛ RIFT [ОБЫЧНЫЙ]", "Полностью рабочий RIFT.", function()
        ...
        workspace.CurrentRooms["100"].Assets.RiftSpawn.Rift.RiftPrompt.Enabled = true
    end)
    PlrSection:NewButton("ОТКРЫТЬ ПОРТАЛ RIFT [THE BACK DOOR]", "Полностью рабочий RIFT.", function()
        ...
        workspace.CurrentRooms["100"].Assets.RiftSpawn.Rift.StarCenter.StarRiftPrompt.Enabled = true
    end)
    ...
    PlrSection:NewButton("МОМЕНТАЛЬНО ОТКРЫВАТЬ ДВЕРИ", "1 раз нажмите, и откроете дверь на замке.", function()
        local function processRooms()
        for i = 0, 100 do
            local roomId = tostring(i)
            local success, err = pcall(function()
                local room = workspace.CurrentRooms[roomId]
                if room and room.Door and room.Door.Lock and room.Door.Lock.UnlockPrompt then
                    room.Door.Lock.UnlockPrompt.HoldDuration = 0
                else
                    error("Room or required components not found")
                end
            end)

            if not success then
                warn("Error processing room " .. roomId .. ": " .. err)
            end
        end
    end
    ...
    local PlrSection = PlrTab:NewSection("God Mode : ")

    PlrSection:NewButton("ТЕЛЕПОРТ К ФИГУРЕ", "Используйте после того, как она за Рычит.", function()
        local roomId = "50"
    local room = workspace.CurrentRooms[roomId]
    local part = room and room.FigureSetup and room.FigureSetup.FigureRagdoll and room.FigureSetup.FigureRagdoll.Torso
    ...
    local PlrSection = PlrTab:NewSection("Имеющиеся : ")

    PlrSection:NewButton("ОТКЛЮЧИТЬ СИКА", "Только для вас.", function()
        local function removeTriggerFromRoom(roomNumber)
        ...
        if room then
            ...
            if trigger then
                trigger:Destroy()
                print("Удален из комнаты: " .. roomNumber)
            end
        end
    end

    -- Удаление из дверей 32-41 (с запасом)
    for i = 32, 41 do
        removeTriggerFromRoom(i)
    end

    -- Удаление из дверей 71-84 (с запасом)
    for i = 71, 84 do
        removeTriggerFromRoom(i)
    end
    end)

    PlrSection:NewButton("ОТКЛЮЧИТЬ JAMMIN", "Отключает музыку Джеффа.", function()
    local soundService = game:GetService("SoundService")

    -- Проверяем, существует ли объект по указанному пути
    local objectToDelete = soundService:FindFirstChild("Main")
    if objectToDelete then
        local jammingObject = objectToDelete:FindFirstChild("Jamming")
        if jammingObject then
            -- Удаляем объект
            jammingObject:Destroy()
            print("Объект Jamming удален.")
        else
            print("Объект Jamming не найден.")
        end
    else
        print("Объект Main не найден.")
    end
    ...
    if mainUI then
        ...
        if initiator then
            ...
            if mainGame then
                local health = mainGame:FindFirstChild("Health")
                if health then
                    local jamObject = health:FindFirstChild("Jam")
                    if jamObject then
                        -- Удаляем объект
                        jamObject:Destroy()
                        print("Объект Jam удален.")
                    else
                        print("Объект Jam не найден.")
                    end
                else
                    print("Объект Health не найден.")
                end
            else
                print("Объект Main_Game не найден.")
            end
        else
            print("Объект Initiator не найден.")
        end
    else
        print("Объект MainUI не найден.")
    end
    ...
    if playerGui then
        ...
        print("Объект PlayerGui не найден.")
    end
    end)
